...
const currentMapName = computed(() => {
  return gameStore.currentMap?.map_name || gameStore.currentRole?.current_map_name || '未知区域'
})

const currentMapType = computed(() => {
  if (gameStore.currentMap?.map_type === 'safe' || gameStore.currentRole?.current_map_safe) {
    return '安全区'
  }
  return '危险区'
})
...
const gameStateText = computed(() => {
  ...
  if (gameStore.transferStatus === 1) {
    return `传送中... ${gameStore.transferTime}秒`
  }
  return '准备就绪'
})
</script>
